...
def classify(image_folder, plot_image=False, save_csv_to=None, sitename=None, crop_size=None, random_crop=False, threshold=0.5):
    ...
    imgpath = Path(image_folder)
    imgfolder = os.path.join(f"{imgpath}", "**", '*.jpg') # recursive glob for jpgs in folder and subfolders

    out_dir = imgpath

    out_dir = Path(save_csv_to) if save_csv_to else imgpath
...
# CLI function to run from command line
def cli():
